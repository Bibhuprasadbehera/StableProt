...
def evaluate_ensemble_1stage(models_tm, embs, seqs, tmhmms, ogt_trues, tm_mean, tm_std, device):
    ...
    # Predictive variance = mean aleatoric variance + spread of the seed means. 1/weights.sum()
    # is the standard error of the ensemble mean and shrinks with seed count.
    total_var = vars_stack.mean(dim=0) + ((mus_stack - ens_mu) ** 2).mean(dim=0)
