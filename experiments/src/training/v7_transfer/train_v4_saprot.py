...
def precompute_features(ids, esm2_embeddings, ogt_lookup, tm_lookup, stage1_predictor, device):
    ...
    for i in range(0, len(ids), batch_size):
        ...
        batch_embs = esm2_embeddings[i:i+batch_size].to(device)
...
def train_seed_stage2(mode, seed, data_saprot, data_esm2, ogt_lookup, tm_lookup, device, save_dir, stage1_model_path=None):
    print("\n" + "="*50)
    print(f"SaProt Stage 2 Tm Training | Mode: {mode} | Seed {seed}")
    print("="*50)
    ...
    if stage1_model_path and os.path.exists(stage1_model_path):
        print(f"Loading ESM-2 Stage 1 predictor from {stage1_model_path} for OGT prediction fallback...")
        stage1_predictor = StableProtV7(
            emb_dim=esm2_input_dim,
    ...
    if use_stratified_sampling:
        ...
        print("Using uniform random sampler...")
...
def main():
    ...
    for seed in CONFIG['seeds']:
        ...
        preds = train_seed_stage2(
            mode=args.mode,
            seed=seed,
            data_saprot=data_saprot,
            data_esm2=data_esm2,
            ogt_lookup=ogt_lookup,
            tm_lookup=tm_lookup,
            device=device,
            save_dir=save_dir,
    ...
    print("\n" + "="*50)
    print(f"Overall results for Mode {args.mode} across seeds: {maes}")
